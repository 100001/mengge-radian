...
define('echarts/chart/pie/pieLayout', ['require', '../../util/number', './labelLayout', 'zrender/core/util'], function (require) {
    ...
    return function (seriesType, ecModel, api, payload) {
        ecModel.eachSeriesByType(seriesType, function (seriesModel) {
            ...
            var menggeRadianSum = 0;
            data.each('value', function (value,i) {
                if(!isNaN(value)){
                     validDataCount++;
                     var mgr = data.getRawDataItem(i).menggeRadian;
                     if(!isNaN(mgr)){
                         menggeRadianSum+=mgr;
                     }
                }
            ...
            // Sum may be 0
            var unitRadian = Math.PI / (menggeRadianSum || sum || validDataCount) * 2;
            ...
            data.each('value', function (value, idx) {
                ...
                if (roseType !== 'area') {
                    if(menggeRadianSum){
                        angle = data.getRawDataItem(idx).menggeRadian * unitRadian;
                    }else
